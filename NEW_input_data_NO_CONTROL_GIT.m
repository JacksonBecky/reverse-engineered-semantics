...
%Layer 2) - Take average in HL1 and HL2 and whichever is highest is conceptual
...
%check examples dir - where saved result of examples script for no control
examples_dir = 'C:\lens\my_scripts\hierarchical_model\example_structure\FIXED_EX\fixed_ex'; % needs examples scripts ran first
example_workspace_name = strcat(examples_dir, '\examples_UNITBASED_CORRELATION_MATRICES_workspace');
%--------

no_runs = 8;  %this version (with control) can only do 80 runs    
control =0;% ONLY WORKS WITHOUT CONTROL - leave as 0
num_ex = 48; %full number of examples * mod of input for no control
...
%% compare vectors to example script - LOOPS THROUGH DIFF EXAMPLE STRUCTURES FOR NO CONTROL
...
%% making full tables of diff results
...
if deep==3;
           dist_from_fuller_structure_full_table(19,1)=av_results_vs_all_examples(19,21); %hu4
            dist_from_fuller_structure_full_table(20,1)=av_results_vs_all_examples(19,19);%all hu
end
...
if deep ==2;
AV_OVER_RUNS_FOR_TABLES_dist_from_fuller_structure_full_table = zeros(19,1);
end
if deep ==3;
AV_OVER_RUNS_FOR_TABLES_dist_from_fuller_structure_full_table = zeros(20,1);
end
...
%% save workspace
 save('sim_matrices_results_workspace_80runs_NO_CONTROL') %change if change number of runs
